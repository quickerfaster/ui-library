...
| L5 | [`src/Http/Livewire/DataTables/DataTableForm.php`](src/Http/Livewire/DataTables/DataTableForm.php:63) | Fixed `company_id` field always visible on forms. The `loadConfiguration()` and `validateFields()` methods only handled the "All Companies → show" path but never added `company_id` to hidden fields when a specific company was selected. Added `else` blocks that add `company_id` to `hiddenFields` in all contexts (create, edit, view). | ✅ Done |
| L6 | [`src/Http/Livewire/Wizards/WizardForm.php`](src/Http/Livewire/Wizards/WizardForm.php:56) | Same `company_id` visibility fix as DataTableForm — added `else` blocks in `loadConfiguration()` and `validateFields()` to hide `company_id` when a specific company is selected. | ✅ Done |
| L7 | [`src/Core/Admin/Config/navigation.php`](src/Core/Admin/Config/navigation.php:156) | Added `user_company_assignments` navigation item in the `Users` context group (order 6), after User Preferences (order 5). Uses route `/admin/user-company-assignments` with permission `manage_user_company_assignments`. This places the assignment UI in the Admin module's Users context rather than the System module's Accounts context, which is the more appropriate location for user management operations. | ✅ Done |
| L8 | [`src/Core/Admin/Data/user.php`](src/Core/Admin/Data/user.php:204) | Added `moreActions` entry with "Manage Companies" label, `fa-solid fa-building` icon, route `admin.user-company-assignments`, and `routeParam` of `user`. This provides a per-user row action in the Users DataTable that links directly to the company assignment page for that specific user. | ✅ Done |
...
| [`src/Http/Livewire/DataTables/DataTableForm.php`](src/Http/Livewire/DataTables/DataTableForm.php:63) | Fixed `company_id` field always visible on forms. `loadConfiguration()` and `validateFields()` now add `company_id` to `hiddenFields` when a specific company is selected (not "All Companies"). Previously only the "All Companies → show `company_id`" path was handled; the `else` block was missing. | 63–86, validateFields |
...
The `user_company_assignments` navigation item follows **Option A** (dedicated sidebar link) as recommended in Section 4 of this document, but has been relocated from the System module's `accounts` context group to the **Admin module's `Users` context group**. This is the more appropriate location since user-company assignment is a user management operation, not a system-level account operation.

The library provides **two entry points** for the assignment UI:

1. **Sidebar navigation link** in Admin → Users → Company Assignments (order 6, after User Preferences)
2. **Row action** (`moreActions`) on the Users DataTable — a "Manage Companies" dropdown action that links directly to the assignment page for a specific user via the `user` route parameter
