...
| Component | Alias | Class | Purpose |
|-----------|-------|-------|---------|
| ApprovalActions | `qf.approval-actions` | `ApprovalActions` | Approve/reject/recall action buttons with `displayMode` (`inline`/`banner`/`card`) |
| ApprovalHistoryTimeline | `qf.approval-history-timeline` | `ApprovalHistoryTimeline` | Approval history timeline with `displayMode` (`full`/`compact`/`steps-only`) |
| ApprovalPanel | `qf.approval-panel` | `ApprovalPanel` | Combined actions + timeline panel with `displayMode` (`banner`/`card`/`inline`) |
| ApprovalRequestListView | `qf.approval-request-list-view` | `ApprovalRequestListView` | Filterable list of approval requests with status filter and reactive title |
...
| FilterService | [`src/Services/Filters/FilterService.php`](../../src/Services/Filters/FilterService.php) | Filter application logic. The `between` operator is routed through `whereBetween()` for correct SQL generation. |
...
> **ListWidgetProcessor** ([`src/Widgets/ListWidgetProcessor.php`](../../src/Widgets/ListWidgetProcessor.php)) accepts the `view_all_link_target` config key (`_self` or `_blank`). When `_blank`, the "View all" link automatically gains `rel="noopener noreferrer"`.
