...
**Related files**: [`00-index.md`](../README.md) · [`08-contracts-and-interfaces.md`](./08-contracts-and-interfaces.md) · [`10-settings-and-config.md`](./10-settings-and-config.md) · [`11-extension-guide.md`](./11-extension-guide.md) · [`14-integration-map.md`](./14-integration-map.md) · [`15-gaps-and-recommendations.md`](./15-gaps-and-recommendations.md) · [`26-module-auto-discovery.md`](./26-module-auto-discovery.md)
...
2. **Generic discovery code** that reads those config keys (never a hardcoded business module name): [`ModuleServiceProvider`](../../src/Providers/ModuleServiceProvider.php:30), [`DiscoveryRegistrar`](../../src/Services/Discovery/DiscoveryRegistrar.php), [`NavigationManager`](../../src/Services/Navigation/NavigationManager.php:572), [`ModelDiscovery`](../../src/Services/AccessControl/ModelDiscovery.php:36), [`ApplicationInfo`](../../src/Services/System/ApplicationInfo.php:56), [`ModelConfigRepository`](../../src/Services/Config/ModelConfigRepository.php:104), [`SettingsPanel`](../../src/Http/Livewire/Settings/SettingsPanel.php:59), [`ModuleSelector`](../../src/Http/Livewire/AccessControls/ModuleSelector.php:34).
...
- Contracts are named by **capability**, never by domain: `Workflowable`, `Documentable`, `Reportable`, `Notifiable`, `ApproverResolver`, `WorkspaceResolver`, `CompanyProvider`, `ModuleContract`, `FieldType`, `Widget`, `CalendarEnhancementProvider`.
- Contract methods use **generic nouns** (`getWorkflowDefinitionKey()`, `getDocumentableId()`, `resolveSubjectId()`, `getHolidays()`, `getTeamAbsences()`) — never `getEmployeeId()`, `getInvoiceNumber()`.
- Docblock examples on contracts must cite **at least two unrelated domains** (see §6.4).

**Canonical example — [`CalendarEnhancementProvider`](../../src/Contracts/FieldTypes/CalendarEnhancementProvider.php):** The library's [`DatepickerField`](../../src/Components/FieldTypes/DatepickerField.php) needs holiday and team-absence data to render calendar markers, but must not reference `App\Modules\Holiday\Models\Holiday` or `App\Modules\Leave\Models\LeaveRequest`. The contract defines `getHolidays(Carbon $month): array` and `getTeamAbsences(Carbon $start, Carbon $end): array` — capability names, not domain names. The consuming app binds its implementation (e.g., `LeaveCalendarEnhancementProvider`) in a service provider. The library resolves it from the container and never knows which domain supplied the data.
...
- Extension points are config keys the consuming app overrides, e.g.:
  ...
  - `ui-library.navigation.workspace_resolver`
  - `ui-library.tenancy.column` / `ui-library.tenancy.session_key`
  - `ui-library.discovery.listeners` / `reports` / `workflows` / `cache_ttl`
...
The library discovers business modules **generically** via [`ModuleServiceProvider::discoverBusinessModules()`](../../src/Providers/ModuleServiceProvider.php:28), which scans the configurable path [`app/Modules`](../../src/Config/ui-library.php:65). The centralized [`DiscoveryRegistrar`](../../src/Services/Discovery/DiscoveryRegistrar.php) drives the convention-based registration of five asset types. The library:
...
- Must treat every discovered directory as an opaque module: it reads `Config/navigation.php`, `Config/workflows.php`, `Config/permissions.php`, `Data/*.php`, `Data/notifications.php`, `Routes/web.php`, `Resources/views/`, `Database/Migrations/`, `Listeners/`, `Reports/` **by convention**, never by hardcoded filename that implies a domain.
- Reports are keyed by the consuming app via the [`#[ReportType]`](../../src/Attributes/ReportType.php) attribute (fallback: `const REPORT_TYPE`, then `getReportType()`). The library reads the *declared key*, never a hardcoded report name.

**Allowed:** the *shape* of the discovery contract (`{$directory}/Routes/web.php`, `{$directory}/Listeners/*.php`, `{$directory}/Reports/*.php`) and the `#[ReportType]` key the consuming app supplies.
...
| Listener discovery | `DiscoveryRegistrar` scans `Listeners/` | Provides `Listeners/*.php` |
| Report discovery | `DiscoveryRegistrar` + `#[ReportType]` attribute | Provides `Reports/*.php` |
| Workflow discovery | Deep-merge into `workflows.definitions` | Provides `Config/workflows.php` |
| Permission discovery | CRUD name generation + overrides | Provides `Config/permissions.php` |
| Notification discovery | Template/channel registry | Provides `Data/notifications.php` |
...
| Tenancy scoping | `CompanyScope` + `HasCompanyScope` + config keys | Applies trait / binds provider |
...
**Alternative pattern — Subclass extension:** When a library Livewire component needs domain-specific behavior that cannot be expressed through a contract alone (e.g., overriding multiple methods that query domain models), the library provides a **base class** with `protected` extension points, and the consuming app creates a **subclass** that overrides them. Example: [`WizardForm`](../../src/Http/Livewire/Wizards/WizardForm.php) provides `getAvailableOptions()`, `validateStep()`, and `calculateDuration()` as overridable methods. The Leave module's [`LeaveWizardForm`](hr-consuming-app:app/Modules/Leave/Http/Livewire/LeaveWizardForm.php) extends it and overrides these methods with leave-specific logic (balance checking, conflict detection, leave type resolution). The library never references the subclass; the consuming app registers it as the wizard component for the `employee_self_service` wizard.

**When to use each pattern:**

| Pattern | Use when | Example |
|---------|----------|---------|
| **Contract + binding** | The library needs *data* from the consuming app (holidays, team members, company info) | [`CalendarEnhancementProvider`](../../src/Contracts/FieldTypes/CalendarEnhancementProvider.php) |
| **Subclass extension** | The library provides *behavior* that the consuming app needs to customize with domain logic | [`LeaveWizardForm`](hr-consuming-app:app/Modules/Leave/Http/Livewire/LeaveWizardForm.php) extends [`WizardForm`](../../src/Http/Livewire/Wizards/WizardForm.php) |
...
The following **intentional** seams are the only places consuming-app awareness is permitted, and each is config-driven or convention-shaped:
...
- `ui-library.discovery.*` toggles (capability switches, not domains).
- `ui-library.tenancy.column` / `ui-library.tenancy.session_key` (configurable column/key names, not domain-specific).
- The [`#[ReportType]`](../../src/Attributes/ReportType.php) attribute (a *key string* supplied by the consuming app at discovery time, read by reflection without instantiating the class).
...
- [ ] Discovery code reads only the configurable module path + convention shapes (§3.3), never a hardcoded module/class.
- [ ] The `#[ReportType]` attribute reads only the consuming-app-supplied key.
...
Before writing any library code, consult the [Pre-Coding Architecture Checklist](../consuming-app/pre-coding-checklist.md), particularly **Section C (Before Modifying Library Code)** and **Section B (Subclass Pattern)**.

### Subclass Pattern (Established 2026-09-02)

When domain-specific logic is needed in a library component, use the subclass pattern:

1. Library provides a base class with stub methods (e.g., `WizardForm::getAvailableLeaveTypes()` returns `[]`)
2. Consuming app creates a subclass (e.g., `LeaveWizardForm extends WizardForm`) with the domain implementation
3. Consuming app registers the subclass as a Livewire component in its service provider

**Example**: [`LeaveWizardForm`](../../app/Modules/Leave/Http/Livewire/LeaveWizardForm.php) extends [`WizardForm`](../../src/Http/Livewire/Wizards/WizardForm.php) to provide leave-specific validation and dynamic field options.

---

**Related files**: [`00-index.md`](../README.md) · [`08-contracts-and-interfaces.md`](./08-contracts-and-interfaces.md) · [`10-settings-and-config.md`](./10-settings-and-config.md) · [`11-extension-guide.md`](./11-extension-guide.md) · [`14-integration-map.md`](./14-integration-map.md) · [`15-gaps-and-recommendations.md`](./15-gaps-and-recommendations.md)
