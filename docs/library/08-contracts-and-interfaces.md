...
**Related files**: [`00-index.md`](../README.md) · [`07-component-catalog.md`](./07-component-catalog.md) · [`09-engines-and-services.md`](./09-engines-and-services.md) · [`11-extension-guide.md`](./11-extension-guide.md) · [`14-integration-map.md`](./14-integration-map.md)
...
**Default implementation**: [`WorkspaceScopedApproverResolver`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php) — resolves approvers within a single workspace scope using Spatie roles and a `company_id` column on the User model. The legacy [`DefaultApproverResolver`](../../src/Services/Approvals/DefaultApproverResolver.php) (global resolution, no workspace filter) is also available for single-tenant apps.

### ApproverLabelResolver Contract

**Location**: [`src/Contracts/Approvals/ApproverLabelResolver.php`](../../src/Contracts/Approvals/ApproverLabelResolver.php:5)
...
interface ApproverLabelResolver
{
    ...
    /** Get the avatar URL for a user ID, or null. */
    public function avatar($userId): ?string;

    /** Get the profile route for a user ID, or null. */
    public function profileRoute($userId): ?string;
}
```

**Default implementation**: [`DefaultApproverLabelResolver`](../../src/Services/Approvals/DefaultApproverLabelResolver.php) — probes `name` → `full_name` → `email` for labels; `avatar_url` → `avatar` → `profile_photo_url` → `photo` for avatars; returns `null` for profile routes.
