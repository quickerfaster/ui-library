...
**Related files**: [`05-data-configs.md`](./05-data-configs.md) · [`06-navigation-system.md`](./06-navigation-system.md) · [`09-engines-and-services.md`](./09-engines-and-services.md) · [`13-adr.md`](./13-adr.md) · [`14-integration-map.md`](./14-integration-map.md) · [`26-module-auto-discovery.md`](./26-module-auto-discovery.md)

> **Consuming-app developers**: For the installation and config publishing guide, see [../consuming-app/getting-started.md](../consuming-app/getting-started.md) §"Configuration & Publishing". For tenancy configuration guidance, see [../consuming-app/multi-tenancy.md](../consuming-app/multi-tenancy.md) §"Tenancy Configuration".
...
Controls the convention-based auto-discovery of business-module assets under `app/Modules/*`. Each global toggle defaults to `true`; per-module opt-outs live on the module registry entry (`ui-library.modules.{module}.auto_register_listeners`, `auto_register_reports`, `auto_register_workflows`, `auto_register_permissions`, `auto_register_notifications`).

- **`listeners`** — scan each module's `Listeners/` directory and auto-register discovered listeners.
- **`reports`** — scan each module for classes implementing `Reportable` and register them into `reports.report_types`.
- **`workflows`** — deep-merge each module's `Config/workflows.php` into `workflows.definitions`.
- **`cache_ttl`** — finite cache lifetime (seconds) for production discovery caches. Cache keys are content-hashed from file paths + mtimes (self-invalidating on deploy); the TTL is a safety net (the library never uses `forever()`).

> **Cross-link**: The full discovery contract, conventions, and the `ui-library:discover` command are documented in [`26-module-auto-discovery.md`](./26-module-auto-discovery.md).
...
- **`approver_resolver`** / **`approver_label_resolver`** — bind the approver-resolution contracts; defaults are [`DefaultApproverResolver`](../../src/Services/Approvals/DefaultApproverResolver.php) and [`DefaultApproverLabelResolver`](../../src/Services/Approvals/DefaultApproverLabelResolver.php). Consuming apps publish the config and point these at their own implementations.
- **`models`** — library-owned model defaults. Consuming apps can override them by publishing the config. The [`ApprovalModelResolver`](../../src/Services/Approvals/ApprovalModelResolver.php) reads from these keys.
...
'workflows' => [
    'definitions' => [
        ...
        //         ['name' => 'Finance Review', 'step_type' => 'approval', 'approval_mode' => 'any', 'roles' => ['finance']],
        //     ],
        // ],
    ],
],
...
> **Cross-link**: The full workflow definition schema and architecture live in [`09-engines-and-services.md`](./09-engines-and-services.md) (Workflow Engine deep-dive).
...
Controls the tenant column and session key used by [`CompanyScope`](../../src/Scopes/CompanyScope.php) and the [`HasCompanyScope`](../../src/Traits/HasCompanyScope.php) trait. "company" is the library's domain-agnostic tenant term (already used by `CompanyProvider`, the company switcher, and the `company_id` convention).

- **`column`** — the Eloquent column the global scope filters on.
- **`session_key`** — the session key holding the currently selected company ID, set by the [`ResolveCompanyContext`](../../src/Http/Middleware/ResolveCompanyContext.php) middleware (alias `qf.resolve-company-context`).
...
**Related files**: [`05-data-configs.md`](./05-data-configs.md) · [`06-navigation-system.md`](./06-navigation-system.md) · [`09-engines-and-services.md`](./09-engines-and-services.md) · [`13-adr.md`](./13-adr.md) · [`14-integration-map.md`](./14-integration-map.md)
