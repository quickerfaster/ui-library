...
> **Status**: Reference (2026-08-30) — the library now ships [`WorkspaceScopedApproverResolver`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php) as the **default**; this doc explains how it works, when you need to override it, and how to write a custom resolver
> **Scope**: How the [`ApproverResolver`](../../src/Contracts/Approvals/ApproverResolver.php) contract works, what the default [`WorkspaceScopedApproverResolver`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php) does, and when a consuming app needs a custom implementation.

**Related files**: [`08-contracts-and-interfaces.md`](../library/08-contracts-and-interfaces.md) · [`09-engines-and-services.md`](../library/09-engines-and-services.md) · [`18-workflow-approval-testing-checklist.md`](./18-workflow-approval-testing-checklist.md) · [`19-notification-consuming-app-guide.md`](./19-notification-consuming-app-guide.md)
...
### 1.1 The Library Now Ships a Workspace-Scoped Default

As of 2026-08-30, the library's default resolver is [`WorkspaceScopedApproverResolver`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php) — it resolves approvers within a single workspace scope, preventing cross-workspace approval leakage. The old [`DefaultApproverResolver`](../../src/Services/Approvals/DefaultApproverResolver.php) (global Spatie resolution, no workspace filter) is still available for single-tenant apps.

**Most consuming apps do not need to write a custom resolver.** The default works out of the box when:

1. Your `User` model has a `company_id` column (or a configurable foreign key column).
2. You use Spatie's role/permission package with the standard `model_has_roles` pivot.

You only need a custom `ApproverResolver` when your User model does **not** have a direct workspace column — for example, when the user→workspace relationship goes through an intermediary model (e.g., `User → Employee → Company`). In that case, see §4 for the reference implementation pattern.

### 1.2 How It Works

The resolver constrains role resolution to one workspace, so approval authority is derived from *role **and** membership*. The resolver does **not** read the record directly. The engine extracts the workspace identifier from the workflow context and passes it to the resolver as the `$workspaceId` argument. The resolver's only job is to honor that scope while expanding roles into user IDs.
...
## 3. Default Behavior — WorkspaceScopedApproverResolver

**Location**: [`src/Services/Approvals/WorkspaceScopedApproverResolver.php`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php)

The library's default resolver is a Spatie-permission-based implementation with workspace scoping:
...
5. Returns `array_values(array_unique($userIds))`.
6. When no workspace-scoped approvers exist, returns empty (safe default) — no cross-workspace leakage.

### 3.1 When the default works out of the box

The default resolver works without any customization when:

- ✅ Your `User` model has a `company_id` column (configurable via `config('app.workspace_approvals.foreign_key')`).
- ✅ You use Spatie's role/permission package with the standard `model_has_roles` pivot.
- ✅ Your workflowable entities return `workspace_id` in their `getWorkflowContext()`.

### 3.2 When you need a custom resolver

You need a custom `ApproverResolver` when your User model does **not** have a direct workspace column. For example:

- The user→workspace relationship goes through an intermediary model (e.g., `User → Employee → Company`).
- Workspace membership is determined by a pivot table rather than a column on the users table.
- You use a non-Spatie role system.

In these cases, override `belongsToWorkspace()` and `activeWorkspaceId()` in a custom resolver (see §4).

### 3.3 The legacy DefaultApproverResolver

The old [`DefaultApproverResolver`](../../src/Services/Approvals/DefaultApproverResolver.php) is still available for single-tenant apps or apps that want global role resolution. It resolves role names globally with no workspace filter. To use it, set `config('ui-library.approvals.approver_resolver')` to `QuickerFaster\UILibrary\Services\Approvals\DefaultApproverResolver::class`.

---

## 4. Custom Resolver — When You Need One

If your User model does not have a direct workspace column (e.g., `company_id`), you need a custom resolver. The library's [`WorkspaceScopedApproverResolver`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php) serves as the reference — override `belongsToWorkspace()` and `activeWorkspaceId()` to match your app's membership model.

The following class is a complete, copy-pasteable starting point for a custom resolver. It is **domain-agnostic** — it speaks in terms of *workspace*, *record*, *entity*, and *organization*, and makes no reference to any specific business domain.

Place it at `app/Approvals/WorkspaceScopedApproverResolver.php` (adjust the namespace to your app). This overrides the library's default.
...
The contract is bound in [`UILibraryServiceProvider::register()`](../../src/Providers/UILibraryServiceProvider.php), reading `config('ui-library.approvals.approver_resolver')` and defaulting to [`WorkspaceScopedApproverResolver`](../../src/Services/Approvals/WorkspaceScopedApproverResolver.php):
...
$this->app->bind(
    ...
    config(
        'ui-library.approvals.approver_resolver',
        \QuickerFaster\UILibrary\Services\Approvals\WorkspaceScopedApproverResolver::class
    )
);
```

**Most consuming apps do not need to change this binding.** The default `WorkspaceScopedApproverResolver` works when your User model has a `company_id` column.

### 5.1 When you need to override

You only need to override the binding when your User model does **not** have a direct workspace column. There are two supported ways. **Pick one.**

#### Config override (recommended)

Publish the library config, then set the key to your custom class:
...
'approvals' => [
    'approver_resolver' => \App\Approvals\YourCustomApproverResolver::class,
    // ...
],
```
...
#### Direct re-binding in a service provider
...
use App\Approvals\YourCustomApproverResolver;
...
class ApproverResolutionServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->bind(ApproverResolver::class, YourCustomApproverResolver::class);
    }
}
```
...
> Use **config override** when you simply swap implementations; use **re-binding** when the class has constructor dependencies you need the container to resolve.

### 5.2 Switching back to global resolution

If you want the old global resolution behavior (no workspace scoping), set the config to the legacy resolver:
